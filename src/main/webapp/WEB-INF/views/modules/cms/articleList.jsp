...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cms/article/?category.id=${article.category.id}">问题列表</a></li>
		<!--  
		<shiro:hasPermission name="cms:article:edit"><li><a href="<c:url value='${fns:getAdminPath()}/cms/article/form?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>">问题添加</a></li></shiro:hasPermission>
		-->
	</ul>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th>栏目</th><th>问题标题</th><th>点击数</th><th>发布者</th><th>更新时间</th><th>操作</th></tr></thead>
		<tbody>
		<c:forEach items="${page.list}" var="article">
			<tr>
				...
				<td><a title="${article.title}">${fns:abbr(article.title,40)}</a></td>
				<td>${article.hits}</td>
				...
				<td>
					...
					<shiro:hasPermission name="cms:article:edit">
						...
						<shiro:hasPermission name="cms:article:audit">
	    				<a href="${ctx}/cms/article/form?id=${article.id}">回答</a>
